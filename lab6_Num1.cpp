...
int main() {
    ...
    BinaryOperation* minus = new BinaryOperation(n32, BinaryOperation::MINUS, n16);  // 16

    FunctionCall* callSqrt = new FunctionCall("sqrt", minus);  // sqrt(16) = 4

    Number* n2 = new Number(2.0);  // ← вместо переменной
    BinaryOperation* mult = new BinaryOperation(n2, BinaryOperation::MUL, callSqrt);  // 2 * 4 = 8

    FunctionCall* callAbs = new FunctionCall("abs", mult);  // abs(8) = 8

    cout << "Оригинал evaluate() = " << callAbs->evaluate() << endl;

    CopySyntaxTree cst;
    Expression* newExpr = callAbs->transform(&cst);

    cout << "Копия evaluate() = " << newExpr->evaluate() << endl;

    cout << "\nПроверка: оригинал и копия - разные объекты в памяти" << endl;
